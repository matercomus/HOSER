...
PLOT_REGISTRY = {
    ...
    'heatmaps': {
        'module': application_plots,
        'functions': ['plot_improvement_heatmaps_individual', 'plot_improvement_heatmap_grid'],
...
def generate_plots(data, output_dir, dpi, plot_groups):
    ...
    for plot_key, plot_config in PLOT_REGISTRY.items():
        ...
        logger.info(f"  {plot_config['name']} ({plot_config['count']} plots: {plot_config['ids']})...")
...
Available plot groups: {available_options}
    ...
    generated = generate_plots(data, output_dir, args.dpi, plot_groups)
