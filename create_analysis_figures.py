...
class EvaluationVisualizer:
    ...
    def __init__(self, eval_dir: str, dataset: str = None):
        ...
        self.figures_dir = self.eval_dir / "figures" / "analysis"
    ...
    def _parse_model_info(self, result: Dict) -> Tuple[str, str]:
        ...
        if model_type == "unknown":
            ...
            if "distill_phase2_seed44" in gen_file:
                model_type = "distill_phase2_seed44"
            elif "distill_phase2_seed43" in gen_file:
                model_type = "distill_phase2_seed43"
            elif "distill_phase2" in gen_file:
                model_type = "distill_phase2"
            elif "distill_phase1_seed44" in gen_file:
                model_type = "distill_phase1_seed44"
            elif "distill_phase1_seed43" in gen_file:
                model_type = "distill_phase1_seed43"
            elif "distill_phase1" in gen_file:
                model_type = "distill_phase1"
            # Beijing convention (distilled with seeds)
            elif "distilled_seed44" in gen_file:
                model_type = "distilled_seed44"
            elif "distilled_seed43" in gen_file:
                model_type = "distilled_seed43"
            elif "distilled" in gen_file:
                model_type = "distilled"
            # Vanilla with seeds (both conventions)
            elif "vanilla_seed44" in gen_file:
                model_type = "vanilla_seed44"
            elif "vanilla_seed43" in gen_file:
                model_type = "vanilla_seed43"
    ...
    def plot_distance_distributions(self):
        ...
        plt.tight_layout()
        plt.savefig(
            self.figures_dir / "distance_distributions.pdf",
            bbox_inches="tight",
            dpi=300,
        )
        plt.savefig(
            self.figures_dir / "distance_distributions.png",
            bbox_inches="tight",
            dpi=300,
        )
        plt.close()
    ...
    def plot_od_matching_rates(self):
        ...
        ax.set_title(
            "OD Pair Coverage: Percentage of Generated OD Pairs Matching Real Data"
        )
        ax.set_xticks(x_pos)
        ...
        plt.tight_layout()
        plt.savefig(
            self.figures_dir / "od_matching_rates.pdf", bbox_inches="tight", dpi=300
        )
        plt.savefig(
            self.figures_dir / "od_matching_rates.png", bbox_inches="tight", dpi=300
        )
        plt.close()
    ...
    def plot_jsd_comparison(self):
        ...
        for od_source in ["train", "test"]:
            ...
            ax1.bar(
                x_pos + offset, values, 0.35, label=f"{od_source.title()} OD", alpha=0.8
            )
        ...
        plt.tight_layout()
        plt.savefig(
            self.figures_dir / "jsd_comparison.pdf", bbox_inches="tight", dpi=300
        )
        plt.savefig(
            self.figures_dir / "jsd_comparison.png", bbox_inches="tight", dpi=300
        )
        plt.close()
    ...
    def plot_metrics_heatmap(self):
        ...
        cbar.set_label(
            "Normalized Score\n(0=best, 1=worst for this metric)",
            rotation=270,
            labelpad=20,
        )
        ...
        plt.tight_layout()
        plt.savefig(
            self.figures_dir / "metrics_heatmap.pdf", bbox_inches="tight", dpi=300
        )
        plt.savefig(
            self.figures_dir / "metrics_heatmap.png", bbox_inches="tight", dpi=300
        )
        plt.close()
    ...
    def plot_train_test_comparison(self):
        ...
        plt.tight_layout()
        plt.savefig(
            self.figures_dir / "train_test_comparison.pdf", bbox_inches="tight", dpi=300
        )
        plt.savefig(
            self.figures_dir / "train_test_comparison.png", bbox_inches="tight", dpi=300
        )
        plt.close()
    ...
    def plot_seed_robustness(self):
        ...
        plt.tight_layout()
        plt.savefig(
            self.figures_dir / "seed_robustness.pdf", bbox_inches="tight", dpi=300
        )
        plt.savefig(
            self.figures_dir / "seed_robustness.png", bbox_inches="tight", dpi=300
        )
        plt.close()
    ...
    def plot_local_metrics(self):
        ...
        plt.tight_layout()
        plt.savefig(
            self.figures_dir / "local_metrics.pdf", bbox_inches="tight", dpi=300
        )
        plt.savefig(
            self.figures_dir / "local_metrics.png", bbox_inches="tight", dpi=300
        )
        plt.close()
    ...
    def plot_performance_radar(self):
        ...
        plt.tight_layout()
        plt.savefig(
            self.figures_dir / "performance_radar.pdf", bbox_inches="tight", dpi=300
        )
        plt.savefig(
            self.figures_dir / "performance_radar.png", bbox_inches="tight", dpi=300
        )
        plt.close()


class ScenarioVisualizer:
    """Create scenario-based visualization plots"""

    def __init__(self, eval_dir: str):
        self.eval_dir = Path(eval_dir).resolve()
        self.scenarios_dir = self.eval_dir / "scenarios"
        self.output_dir = self.eval_dir / "figures" / "scenarios"
        self.output_dir.mkdir(parents=True, exist_ok=True)

    def plot_from_results(self, results: Dict, model_name: str, od_source: str):
        """Create all scenario plots from analysis results"""
        output_subdir = self.output_dir / od_source / model_name
        output_subdir.mkdir(parents=True, exist_ok=True)

        self.plot_scenario_distribution(results, output_subdir)
        self.plot_metric_heatmap(results, output_subdir)
        self.plot_hierarchical_breakdown(results, output_subdir)

    def plot_scenario_distribution(self, results: Dict, output_dir: Path):
        """Create scenario distribution bar chart"""
    ...
    def plot_metric_heatmap(self, results: Dict, output_dir: Path):
        """Create metric comparison heatmap"""
        scenario_metrics = results.get("scenarios", {})
        if not scenario_metrics:
            return

        # Extract metrics into matrix
        metric_names = ["Distance JSD", "Duration JSD", "Hausdorff", "DTW", "EDR"]
        ...
        for scenario in scenarios:
            ...
            row.append(metrics.get("distance_jsd", 0))
            row.append(metrics.get("duration_jsd", 0))
            row.append(metrics.get("hausdorff_distance", 0))
            row.append(metrics.get("dtw_distance", 0))
            row.append(metrics.get("edr_distance", 0))
            matrix.append(row)

        matrix = np.array(matrix)

        # Normalize per metric (column)
        matrix_norm = matrix / (matrix.max(axis=0) + 1e-10)

        plt.figure(figsize=(10, 8))
        plt.imshow(matrix_norm, cmap="YlOrRd", aspect="auto")
    ...
    def plot_hierarchical_breakdown(self, results: Dict, output_dir: Path):
        """Create pie charts for hierarchical breakdowns"""
        hierarchical = results.get("overview", {}).get("hierarchical_breakdown", {})
        if not hierarchical:
            return

        # Create one pie chart per parent scenario
        for parent_scenario, breakdown in hierarchical.items():
            ...
            percentages = 100.0 * np.array(counts) / sum(counts)

            plt.pie(
                counts,
                labels=[
                    f"{child}\n({pct:.1f}%)"
                    for child, pct in zip(child_scenarios, percentages)
                ],
                colors=colors,
                autopct="%1.0f",
                startangle=90,
            ...
            plt.title(
                f"Breakdown of {parent_scenario.upper()} Trajectories", fontsize=16
            )
            plt.tight_layout()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate analysis figures for HOSER evaluation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Generate analysis figures from evaluation directory
  uv run python create_analysis_figures.py --eval-dir hoser-distill-optuna-6
  
  # Specify dataset explicitly
  uv run python create_analysis_figures.py --eval-dir eval_xyz --dataset Porto
        """,
    )
    parser.add_argument(
