...
class MyCollateFn:
    def __init__(
        self,
        timestamp_label_log1p_mean,
        timestamp_label_log1p_std,
...
def main(
    ...
    if dataset is None:
        ...
        parser.add_argument(
            ...
            type=str,
            default="",
            help="Path to YAML config (overrides dataset default path)",
        )
        ...
        parser.add_argument(
            ...
            type=str,
            default="",
            help="Path to HOSER-format data directory (overrides YAML)",
        )
        parser.add_argument(
            "--return_metrics",
            action="store_true",
            help="Return validation metrics (for Optuna)",
        )
        parser.add_argument(
            "--profile_max_batches",
            type=int,
    ...
    if not os.path.exists(_config_path):
        raise FileNotFoundError(
            f"Config not found at {_config_path}. Ensure the dataset YAML exists."
        )
    ...
    dataset_name = (
        dataset if dataset else os.path.splitext(os.path.basename(_config_path))[0]
    )

    cfg_data_dir = (
        getattr(config, "data_dir", None) if hasattr(config, "data_dir") else None
    )
    base_data_dir = (
        data_dir
        if data_dir
        else (cfg_data_dir if cfg_data_dir else f"./data/{dataset_name}")
    )
    ...
    road_attr_lon = np.array(
        [
            LineString(coordinates=eval(row["coordinates"])).centroid.x
            for _, row in geo.iterrows()
        ]
    ).astype(np.float32)
    road_attr_lat = np.array(
        [
            LineString(coordinates=eval(row["coordinates"])).centroid.y
            for _, row in geo.iterrows()
        ]
    ).astype(np.float32)
    ...
    min_lng = float(
        np.min(
            [min([p[0] for p in eval(row["coordinates"])]) for _, row in geo.iterrows()]
        )
    )
    max_lng = float(
        np.max(
            [max([p[0] for p in eval(row["coordinates"])]) for _, row in geo.iterrows()]
        )
    )
    min_lat = float(
        np.min(
            [min([p[1] for p in eval(row["coordinates"])]) for _, row in geo.iterrows()]
        )
    )
    max_lat = float(
        np.max(
            [max([p[1] for p in eval(row["coordinates"])]) for _, row in geo.iterrows()]
        )
    )
    ...
    for road_id in range(num_roads):
        for adj_road_id in list(road_adj_lists[road_id]):
            ...
            road_id_angle = get_angle(
                road_id_coord[0][1],
                road_id_coord[0][0],
                road_id_coord[-1][1],
                road_id_coord[-1][0],
            )
            adj_road_id_angle = get_angle(
                adj_road_id_coord[0][1],
                adj_road_id_coord[0][0],
                adj_road_id_coord[-1][1],
                adj_road_id_coord[-1][0],
            )
    ...
    dl_persist = bool(
        getattr(getattr(config, "dataloader", {}), "persistent_workers", False)
    )
    ...
    train_dataloader = DataLoader(
        ...
        collate_fn=MyCollateFn(
            timestamp_mean,
            timestamp_std,
            getattr(config.trajectory_encoder_config, "max_len", None),
        ),
    ...
    cudnn_bench = bool(
        getattr(getattr(config, "training", {}), "cudnn_benchmark", False)
    )
    compile_flag = bool(
        getattr(getattr(config, "training", {}), "torch_compile", False)
    )
    ...
    disable_cudagraphs = bool(
        getattr(getattr(config, "training", {}), "disable_cudagraphs", False)
    )
    if compile_flag:
        try:
            ...
            model = torch.compile(model, mode=compile_mode, disable=disable_cudagraphs)
    ...
    for epoch_id in range(config.optimizer_config.max_epoch):
        ...
        for batch_id, (
            ...
            batch_trace_distance_mat = batch_trace_distance_mat.to(
                device, non_blocking=True
            )
            batch_trace_time_interval_mat = batch_trace_time_interval_mat.to(
                device, non_blocking=True
            )
            batch_trace_len = batch_trace_len.to(device, non_blocking=True)
            batch_destination_road_id = batch_destination_road_id.to(
                device, non_blocking=True
            )
            ...
            if top_k > 0:
                ...
                with torch.no_grad():
                    ...
                    batch_metric_dis_gpu = batch_metric_dis.to(
                        device, non_blocking=True
                    )
                    batch_metric_angle = batch_metric_angle.to(
                        device, non_blocking=True
                    )
                    batch_candidate_len_gpu = batch_candidate_len.to(
                        device, non_blocking=True
                    )
                    ...
                    sorted_indices = torch.argsort(
                        batch_metric_dis_gpu, dim=-1, descending=False
                    )

                    # Create a mask for valid candidates (within candidate_len)
                    _, _, C = batch_candidate_road_id.shape
                    indices = (
                        torch.arange(C, device=device).unsqueeze(0).unsqueeze(0)
                    )  # [1, 1, C]
                    valid_mask = indices < batch_candidate_len_gpu.unsqueeze(
                        -1
                    )  # [B, T, C]
                    ...
                    batch_candidate_road_id = torch.gather(
                        batch_candidate_road_id, -1, idx
                    )
                    batch_metric_dis = torch.gather(batch_metric_dis_gpu, -1, idx)
                    batch_metric_angle = torch.gather(batch_metric_angle, -1, idx)
                    batch_candidate_len = torch.clamp(
                        batch_candidate_len.to(device, non_blocking=True), max=k
                    )

                    # Update road labels to match the filtered candidates
                    # Vectorized approach for efficiency
                    batch_road_label_device = batch_road_label.to(
                        device, non_blocking=True
                    )
                    ...
                    idx_expanded = sorted_indices[
                        ..., :k
                    ]  # [B, T, k] - use original sorted indices
            ...
            batch_trace_grid_token = batch_trace_grid_token.to(
                device, non_blocking=True
            )
            batch_candidate_grid_token = batch_candidate_grid_token.to(
                device, non_blocking=True
            )
            ...
            with torch.amp.autocast(device_type="cuda"):
                ...
                logits_mask = torch.arange(
                    logits.size(1), dtype=torch.int64, device=device
                ).unsqueeze(0) < batch_trace_len.unsqueeze(1)
                ...
                candidate_mask = torch.arange(
                    selected_logits.size(1), dtype=torch.int64, device=device
                ).unsqueeze(0) < selected_candidate_len.unsqueeze(1)
                masked_selected_logits = selected_logits.masked_fill(
            ...
            if distill_mgr is not None:
                ...
                loss = (
                    loss_next_step
                    + loss_time_pred
                    + distill_mgr.cfg.lambda_kl * kl_loss
                )
            else:
                kl_loss = torch.tensor(0.0, device=device)
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                print(
                    f"❌ Critical: NaN/inf total loss detected! loss_next_step={loss_next_step.item():.4f}, loss_time_pred={loss_time_pred.item():.4f}, kl_loss={kl_loss.item():.4f}"
                )
            ...
            if ((batch_id + 1) % accum_steps) == 0:
                ...
                nn.utils.clip_grad_norm_(
                    model.parameters(), config.optimizer_config.max_norm
                )
            ...
            if profile_max_batches and (batch_id + 1) >= profile_max_batches:
                logger.info(
                    f"[profile] Reached {profile_max_batches} batches, stopping early"
                )
                break
        ...
        logger.info(
            f"[training+distill] epoch{epoch_id + 1}, loss_next_step {loss_next_step.item():.3f}, loss_time_pred {loss_time_pred.item():.3f}"
            + (f", loss_kl {kl_loss.item():.3f}" if distill_mgr is not None else "")
        )
        ...
        with torch.no_grad():
            for (
                ...
                top_k = int(
                    getattr(getattr(config, "data", {}), "candidate_top_k", 0) or 0
                )
                if top_k > 0:
                    with torch.no_grad():
                        # Move data to GPU first
                        ...
                        # Expand dimensions for broadcasting
                        labels_expanded = batch_road_label_device.unsqueeze(
                            -1
                        )  # [B, T, 1]
                        idx_expanded = sorted_indices[
                            ..., :k
                ...
                batch_candidate_len = batch_candidate_len.to(device, non_blocking=True)
                batch_road_label = batch_road_label.to(device, non_blocking=True)
                batch_timestamp_label = batch_timestamp_label.to(
                    device, non_blocking=True
                )
                batch_trace_grid_token = batch_trace_grid_token.to(
                    device, non_blocking=True
                ...
                val_next_step_correct_cnt += torch.sum(
                    torch.argmax(masked_selected_logits, dim=1) == selected_road_label
                ).item()
        ...
        if math.isnan(val_mape) or math.isinf(val_mape) or val_mape < 0:
            print(
                f"⚠️  Warning: Invalid val_mape detected: {val_mape}, setting to large value"
            )
            val_mape = 1000.0
        ...
        validation_metrics.append(
            {"epoch": epoch_id + 1, "val_acc": val_acc, "val_mape": val_mape}
        )
    ...
    if return_metrics:
        ...
        return {
            ...
            "final_val_acc": validation_metrics[-1]["val_acc"]
            if validation_metrics
            else 0.0,
            "final_val_mape": validation_metrics[-1]["val_mape"]
            if validation_metrics
            else float("inf"),
