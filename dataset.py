...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, geo_file, rel_file, traj_file):
        ...
        if os.path.exists(aggregate_file):
            data = torch.load(aggregate_file, map_location='cpu', mmap=True)
        ...
        # Build aggregate from individual cached files
        file_paths = sorted(
        ...
        # Scan for metadata if not exists
        ...
        max_trace_len = int(metadata['max_trace_len'])
        max_candidate_len = int(metadata['max_candidate_len'])
        ...
        with multiprocessing.Pool(
            ...
            for packed in tqdm(
                ...
                total=int((len(file_paths) + chunk_size - 1) // chunk_size),
        ...
        print(f'✅ Saved aggregate to {aggregate_file}')
