    ...
    def __call__(self, items):
        ...
        for i in range(len(batch_trace_road_id)):
            ...
            for j in range(len(batch_candidate_road_id[i])):
                ...
                batch_candidate_road_id[i][j] = np.pad(
                    batch_candidate_road_id[i][j],
                    (0, candidate_pad_len),
                    "constant",
                    constant_values=0,
                )
                batch_metric_dis[i][j] = np.pad(
                    batch_metric_dis[i][j],
                    (0, candidate_pad_len),
                    "constant",
                    constant_values=0.0,
                )
                batch_metric_angle[i][j] = np.pad(
            ...
            batch_candidate_road_id[i] = np.concatenate(
                (
                    np.stack(batch_candidate_road_id[i]),
                    np.zeros((trace_pad_len, max_candidate_len), dtype=np.int64),
                ),
                axis=0,
            )
            batch_metric_dis[i] = np.concatenate(
                (
                    np.stack(batch_metric_dis[i]),
                    np.zeros((trace_pad_len, max_candidate_len), dtype=np.float32),
                ),
                axis=0,
            )
            batch_metric_angle[i] = np.concatenate(
                (
                    np.stack(batch_metric_angle[i]),
                    np.zeros((trace_pad_len, max_candidate_len), dtype=np.float32),
                ),
                axis=0,
            )
...
if __name__ == "__main__":
    ...
    logger.info(
        f"timestamp_label_array_log1p_mean {timestamp_label_array_log1p_mean:.3f}"
    )
    logger.info(
        f"timestamp_label_array_log1p_std {timestamp_label_array_log1p_std:.3f}"
    )
    ...
    train_dataloader = DataLoader(
        ...
        collate_fn=MyCollateFn(
            timestamp_label_array_log1p_mean, timestamp_label_array_log1p_std
        ),
        num_workers=16,
        pin_memory=False,
    )
    ...
    logger.info(
        f"config.road_network_encoder_config {config.road_network_encoder_config}"
    )
    logger.info(
        f"config.road_network_encoder_feature {config.road_network_encoder_feature}"
    )
    ...
    warmup_iters = (
        config.optimizer_config.max_epoch
        * len(train_dataloader)
        * config.optimizer_config.warmup_ratio
    )
    iter_num = 0
    ...
    for epoch_id in range(config.optimizer_config.max_epoch):
        ...
        for batch_id, (
            ...
            batch_road_label,
            batch_timestamp_label,
        ) in enumerate(tqdm(train_dataloader, desc=f"[training] epoch{epoch_id + 1}")):
            ...
            writer.add_scalar(
                "loss_next_step",
                loss_next_step.item(),
                len(train_dataloader) * epoch_id + batch_id,
            )
            writer.add_scalar(
                "loss_time_pred",
                loss_time_pred.item(),
            ...
            writer.add_scalar(
                "loss", loss.item(), len(train_dataloader) * epoch_id + batch_id
            )
            writer.add_scalar(
                "learning_rate",
        ...
        logger.info(
            f"[training] epoch{epoch_id + 1}, loss_next_step {loss_next_step.item():.3f}, loss_time_pred {loss_time_pred.item():.3f}"
        )
        ...
        for batch_id, (
            ...
            batch_road_label,
            batch_timestamp_label,
        ) in enumerate(tqdm(val_dataloader, desc=f"[validating] epoch{epoch_id + 1}")):
            batch_trace_road_id = batch_trace_road_id.to(device)
            batch_temporal_info = batch_temporal_info.to(device)
            batch_trace_distance_mat = batch_trace_distance_mat.to(device)
            batch_trace_time_interval_mat = batch_trace_time_interval_mat.to(device)
            ...
            with torch.no_grad():
                with torch.amp.autocast(device_type="cuda"):
                    ...
                    val_next_step_correct_cnt += torch.sum(
                        torch.argmax(masked_selected_logits, dim=1)
                        == selected_road_label
                    ).item()
                    val_next_step_total_cnt += torch.sum(batch_trace_len).item()

                    selected_time_pred = time_pred[logits_mask][
                        torch.arange(time_pred[logits_mask].size(0)),
                        selected_road_label,
                    ]
                    selected_time_pred = (
                        selected_time_pred * timestamp_label_array_log1p_std
        ...
        writer.add_scalar(
            "val_next_step_acc",
            val_next_step_correct_cnt / val_next_step_total_cnt,
            len(train_dataloader) * epoch_id + batch_id,
        )
        writer.add_scalar(
            "val_time_pred_mape",
            val_time_pred_mape_sum / val_time_pred_total_cnt,
            len(train_dataloader) * epoch_id + batch_id,
        )
        logger.info(
            f"[validating] epoch{epoch_id + 1}, val_next_step_acc {val_next_step_correct_cnt / val_next_step_total_cnt:.3f}, val_time_pred_mape {val_time_pred_mape_sum / val_time_pred_total_cnt:.3f}"
        )
        metrics_list.append(val_next_step_correct_cnt / val_next_step_total_cnt)

        torch.save(
            model.state_dict(), os.path.join(save_dir, f"epoch_{epoch_id + 1}.pth")
        )

    best_epoch = np.argmax(metrics_list)
    logger.info(f"loading epoch_{best_epoch + 1}.pth")
    best_model_state_dict = torch.load(
        os.path.join(save_dir, f"epoch_{best_epoch + 1}.pth"), map_location=device
    )
    ...
    for batch_id, (
        ...
        batch_road_label,
        batch_timestamp_label,
    ) in enumerate(tqdm(test_dataloader, desc="[testing]")):
        batch_trace_road_id = batch_trace_road_id.to(device)
        batch_temporal_info = batch_temporal_info.to(device)
        batch_trace_distance_mat = batch_trace_distance_mat.to(device)
        batch_trace_time_interval_mat = batch_trace_time_interval_mat.to(device)
        ...
        with torch.no_grad():
            with torch.amp.autocast(device_type="cuda"):
                ...
                test_next_step_correct_cnt += torch.sum(
                ...
                test_time_pred_mape_sum += torch.sum(
                    torch.abs(selected_time_pred - selected_timestamp_label)
                    / torch.clamp(selected_timestamp_label, min=1.0)
                )
    ...
    logger.info(
        f"[testing] test_next_step_acc {test_next_step_correct_cnt / test_next_step_total_cnt:.3f}, test_time_pred_mape {test_time_pred_mape_sum / test_time_pred_total_cnt:.3f}"
    )
