...
def calculate_visit_counts(traj_path, num_roads, is_real_data):
    ...
    print(
        f"📊 Calculating visit counts for {'Real' if is_real_data else 'Generated'} data..."
    )
    traj_df = pd.read_csv(traj_path)
    visit_counts = np.zeros(num_roads, dtype=np.int64)

    for _, row in tqdm(
        traj_df.iterrows(), total=len(traj_df), desc="Processing trajectories"
    ):
        rids = []
        try:
            if is_real_data:
                ...
                rid_str = (
                    row["gene_trace_road_id"].replace("np.int64(", "").replace(")", "")
                )
...
def plot_heatmap(geo_df, visit_counts, title, output_path):
    ...
    roads_gdf.plot(
        ...
        ax=ax,
        vmin=0,
        vmax=np.percentile(visit_counts[visit_counts > 0], 99),
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate heatmaps for trajectory data."
    )
    parser.add_argument(
        "--run_dir",
        type=str,
    ...
    if not generated_traj_path:
        print(
            f"❌ Error: No generated trajectory CSV file found in {hoser_format_path}"
        )
        return
    ...
    real_visit_counts = calculate_visit_counts(
        real_traj_path, num_roads, is_real_data=True
    )
    generated_visit_counts = calculate_visit_counts(
        generated_traj_path, num_roads, is_real_data=False
    )

    # Generate and save plots
    plot_heatmap(
        geo_df, real_visit_counts, "Real", os.path.join(output_dir, "Real.png")
    )
    plot_heatmap(
        geo_df,
        generated_visit_counts,
        "Generated",
