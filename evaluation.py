    ...
    if generated_file:
        ...
        if gen_meta["run_id"]:
            ...
            if gen_meta["project"]:
                ...
                print(
                    f"📊 Detected generation run: {gen_meta['run_name']} (ID: {gen_meta['run_id']})"
                )
                ...
                if item.startswith("run-"):
                    ...
                    if meta["run_id"] and metadata["training_run_id"] is None:
                        ...
                        if meta["project"]:
                            ...
                            print(
                                f"📊 Detected training run: {meta['run_name']} (ID: {meta['run_id']})"
                            )
                        break
...
def load_road_network(geo_path):
    ...
    try:
        geo_df = pl.read_csv(
            geo_path, schema_overrides={"lanes": pl.Utf8, "oneway": pl.Utf8}
        )
    ...
    if missing:
        raise ValueError(
            f"🚨 FATAL: Road network missing required columns: {missing}\nAvailable: {list(geo_df.columns)}"
        )
    ...
    if min_id != 0 or max_id != len(road_ids) - 1:
        print(
            f"⚠️  WARNING: Road IDs not sequential. Min: {min_id}, Max: {max_id}, Count: {len(road_ids)}"
        )
        print(
            f"    This will cause indexing issues. Expected: 0 to {len(road_ids) - 1}"
        )
    ...
    if failed_roads:
        ...
        if len(failed_roads) > len(geo_df) * 0.1:  # >10% failure
            raise RuntimeError(
                f"🚨 FATAL: Too many roads ({len(failed_roads)}) failed centroid calculation!"
            )
...
def load_trajectories(traj_path, is_real_data, max_road_id=None):
    ...
    if is_real_data and cache_key in _TRAJECTORY_CACHE:
        print(
            f"📂 Using cached real trajectories from {os.path.basename(traj_path)}..."
        )
    ...
    if missing:
        raise ValueError(
            f"🚨 FATAL: Trajectory file missing required columns: {missing}\nAvailable: {traj_df.columns}"
        )
                ...
                if max_road_id is not None:
                    ...
                    if invalid_rids:
                        ...
                        if out_of_bounds_count <= 5:  # Show first 5
                            print(
                                f"⚠️  Traj {idx}: road IDs out of bounds: {invalid_rids[:10]}"
                            )
                        continue
    ...
    if invalid_count > 0:
        ...
        if invalid_count > len(traj_df) * 0.1:  # >10% failure
            raise RuntimeError(
                f"🚨 FATAL: Too many trajectories ({invalid_count}) failed parsing!"
            )

    if out_of_bounds_count > 0:
        print(
            f"⚠️  WARNING: {out_of_bounds_count} trajectories have out-of-bounds road IDs"
        )
        if out_of_bounds_count > len(traj_df) * 0.1:  # >10% failure
            raise RuntimeError(
                f"🚨 FATAL: Too many trajectories ({out_of_bounds_count}) have invalid road IDs!"
            )
    ...
    print(
        f"✅ Loaded {len(trajectories):,} valid trajectories (dropped {invalid_count + out_of_bounds_count})"
    )
...
class GlobalMetrics:
    ...
    def _calculate_distance_distribution(self, trajectories):
        ...
        for traj in tqdm(trajectories, desc="Calculating distances"):
            ...
            for i in range(1, len(road_ids)):
                ...
                try:
                    ...
                    print(
                        f"⚠️  Distance calculation error for roads {road_prev}→{road_curr}: {e}"
                    )
                    continue
        ...
        if skipped_trajs > 0:
            print(
                f"⚠️  Skipped {skipped_trajs} trajectories with no valid road segments"
            )
...
class LocalMetrics:
    ...
    def __init__(
        self, real_trajs, generated_trajs, geo_df, grid_size=0.001, edr_eps=100.0
    ):
        self.real_trajs = real_trajs
        self.generated_trajs = generated_trajs
        self.geo_df = geo_df.set_index("road_id")
    ...
    def _setup_grid(self):
        ...
        valid_gps = [
            gps
            for gps in self.road_gps.values()
            if gps is not None and gps[0] is not None and gps[1] is not None
        ]

        if not valid_gps:
            raise RuntimeError(
                "🚨 FATAL: No valid GPS coordinates found for grid setup!"
            )
        ...
        print(
            f"📐 Grid setup: {self.img_width} × {self.img_height} cells ({self.grid_size}° ≈ {grid_size_m:.0f}m resolution)"
        )
        print(
            f"   Bounds: lon [{self.min_lon:.4f}, {self.max_lon:.4f}], lat [{self.min_lat:.4f}, {self.max_lat:.4f}]"
        )
    ...
    def _get_od_key(self, origin_rid, dest_rid):
        ...
        key = (o_rid_x * self.img_height + o_rid_y, d_rid_x * self.img_height + d_rid_y)
        return key
    ...
    def _get_coord_traj(self, trajectory):
        ...
        return np.array(
            [(self.road_gps[rid][1], self.road_gps[rid][0]) for rid in road_ids]
        )
    ...
    def _calculate_edr(self, t0, t1, eps=100):
        ...
        def great_circle_distance(lon1, lat1, lon2, lat2):
            ...
            a = math.sin(dlat / 2.0) * math.sin(dlat / 2.0) + math.cos(
                rad * lat1
            ) * math.cos(rad * lat2) * math.sin(dlon / 2.0) * math.sin(dlon / 2.0)
            ...
            for j in range(1, n1 + 1):
                ...
                if (
                    great_circle_distance(
                        t0[i - 1][1],
                        t0[i - 1][0],  # lon, lat from t0
                        t1[j - 1][1],
                        t1[j - 1][0],  # lon, lat from t1
                    )
                    < eps
                ):
                    subcost = 0
                    ...
                    subcost = 1
                C[i][j] = min(
                    C[i][j - 1] + 1, C[i - 1][j] + 1, C[i - 1][j - 1] + subcost
                )
...
def evaluate_trajectories_programmatic(
    ...
    real_trajectories = load_trajectories(
        real_path, is_real_data=True, max_road_id=max_road_id
    )
    generated_trajectories = load_trajectories(
        generated_file, is_real_data=False, max_road_id=max_road_id
    )

    print(
        f"Loaded {len(real_trajectories)} real and {len(generated_trajectories)} generated trajectories"
    )

    # Run evaluations
    global_metrics = GlobalMetrics(
        real_trajectories, generated_trajectories, geo_df
    ).evaluate()
    ...
    if generation_performance:
        ...
        perf = generation_performance
        print(f"{'Throughput':<25} {perf.get('throughput_traj_per_sec', 0):.2f} traj/s")
        print(f"{'Mean time/trajectory':<25} {perf.get('total_time_mean', 0):.3f}s")
        print(f"{'Min time':<25} {perf.get('total_time_min', 0):.3f}s")
        print(f"{'Max time':<25} {perf.get('total_time_max', 0):.3f}s")
        print(f"{'Std dev':<25} {perf.get('total_time_std', 0):.3f}s")
    ...
    if enable_wandb:
        run_name = (
            wandb_run_name or f"eval-{os.path.basename(generated_file)}-{timestamp}"
        )
        ...
        wandb.init(
            project=wandb_project or "hoser-eval",
            name=run_name,
            tags=wandb_tags or ["eval"],
            config=wandb_config,
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate trajectory generation models."
    )
    parser.add_argument(
        "--run_dir",
        type=str,
        required=True,
        help="Path to the run directory containing hoser_format folder.",
    )
    parser.add_argument(
        "--generated_file",
        type=str,
        help="Path to specific generated CSV file. If not provided, will search for generated files in run_dir/hoser_format/",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="WandB project name (auto-detected from run metadata if not specified)",
    )
    parser.add_argument(
        "--wandb_run_name", type=str, default="", help="WandB run name (optional)"
    )
    parser.add_argument(
        "--wandb_tags", type=str, nargs="*", default=["eval"], help="WandB tags"
    )
    parser.add_argument(
        "--grid_size",
        type=float,
        default=0.001,
        help="Grid size in degrees for OD pair matching (default: 0.001 from Beijing.yaml)",
    )
    parser.add_argument(
        "--edr_eps",
        type=float,
        default=100.0,
        help="EDR threshold in meters (default: 100.0)",
    )
    ...
    if os.path.isdir(hoser_format_path):
        ...
        print(
            f"❌ Error: Neither 'hoser_format' subdirectory nor dataset files found in {args.run_dir}"
        )
        return
    ...
    generated_trajectories = load_trajectories(
        generated_path, is_real_data=False, max_road_id=max_road_id
    )
    ...
    if args.wandb:
        run_name = (
            args.wandb_run_name or f"eval-{os.path.basename(args.run_dir)}-{timestamp}"
        )
        ...
        log_payload = {
            k: v
            for k, v in all_results.items()
            if k != "metadata" and isinstance(v, float)
        }
        wandb.log(log_payload)
        ...
        if wandb_metadata["training_run_id"]:
            print(
                f"🔗 Training run: {wandb_metadata['training_run_name']} (ID: {wandb_metadata['training_run_id']})"
            )
        if wandb_metadata["generation_run_id"]:
            print(
                f"🔗 Generation run: {wandb_metadata['generation_run_name']} (ID: {wandb_metadata['generation_run_id']})"
            )
