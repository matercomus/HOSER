...
def main():
    ...
    for split in ['train', 'val', 'test']:
        ...
        print(f'   This requires Dataset preprocessing to have already run.')
        print(f'   If cache doesn't exist, run Dataset preprocessing first.')
        ...
        if not cache_dir.exists() or not list(cache_dir.glob('data_*.pt')):
            ...
            print(f'   Run preprocessing first by instantiating Dataset from Python:')
            print(f'   >>> from dataset import Dataset')
            print(f'   >>> Dataset("{geo_file}", "{rel_file}", "{traj_file}")')
            continue
        
        print(f'✅ Found individual cache files, building aggregate...')
