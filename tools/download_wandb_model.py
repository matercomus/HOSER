    ...
    try:
        ...
        if verbose:
            ...
            if "distill" in config and "enable" in config["distill"]:
                ...
                print(
                    f"   Distillation: {'✅ Enabled' if distill_enabled else '❌ Disabled (vanilla)'}"
                )

            if (
                "optimizer_config" in config
                and "max_epoch" in config["optimizer_config"]
            ):
...
def search_runs(
    ...
    if distill_enabled is not None:
        filters.append(
            f"   - Distillation: {'enabled' if distill_enabled else 'disabled'}"
        )
    ...
    try:
        ...
        print(
            f"{'ID':<15} {'Name':<30} {'Distill':<10} {'Epochs':<8} {'Val Acc':<10} {'State':<10}"
        )
        print("-" * 90)
        ...
        for run_info in matching_runs:
            ...
            print(
                f"{run_info['id']:<15} {run_info['name']:<30} {distill_str:<10} "
...
def main():
    ...
    parser.add_argument(
        "--project",
        type=str,
        default="hoser-distill-optuna-6",
    ...
    parser.add_argument(
        "--run_id", type=str, help="WandB run ID to download (e.g., 0vw2ywd9)"
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    parser.add_argument(
        "--search", action="store_true", help="Search for runs instead of downloading"
    )
    parser.add_argument(
        "--distill",
        type=str,
    ...
    parser.add_argument("--epochs", type=int, help="Filter by number of epochs")
    parser.add_argument("--min_val_acc", type=float, help="Minimum validation accuracy")
    parser.add_argument(
        "--limit",
        type=int,
        default=10,
    ...
    if args.search:
        ...
        if matching_runs:
            ...
            print("   uv run python tools/download_wandb_model.py --run_id <ID>")
