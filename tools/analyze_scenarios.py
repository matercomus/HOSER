...
class ScenarioCategorizer:
            ...
            for airport in self.config.spatial["airport"]["airports"]:
                ...
                for road_id, coords in self.road_coords.items():
                    if geodesic:
                        ...
                        distance_km = (
                            np.sqrt(lat_diff**2 + lon_diff**2) * 111
                        )  # rough km conversion
    ...
    def categorize_od_pair(
        self, origin_road_id: int, dest_road_id: int, timestamp: datetime
    ) -> Dict:
        """
        ...
        if (
            self.config.spatial["city_center"]["enabled"]
            and origin_coords
            and dest_coords
        ):
        ...
        if self.config.spatial["airport"]["enabled"]:
            ...
            scenarios["is_airport_trip"] = (
                scenarios["origin_is_airport"] or scenarios["dest_is_airport"]
            )
...
class ScenarioAnalyzer:
    ...
    def __init__(self, config: ScenarioConfig, geo_df: pl.DataFrame):
        ...
        self.geo_df_pandas = (
            geo_df_renamed.to_pandas()
        )  # Convert for evaluation metrics
    ...
    def analyze(self, generated_df: pl.DataFrame, real_df: pl.DataFrame) -> Dict:
        ...
        trajectory_metadata = self._build_trajectory_metadata(
        ...
        results = {
            ...
            "trajectory_scenarios": self._build_trajectory_scenarios_mapping(
                scenario_data
            ),
        }
    ...
    def _categorize_all_trajectories(self, generated_df: pl.DataFrame) -> List[Dict]:
        ...
        for row in generated_df.iter_rows(named=True):
            ...
            timestamp = datetime.fromisoformat(
                row["source_origin_time"].replace("Z", "+00:00")
            )
    ...
    def _build_trajectory_metadata(
        self, scenario_data: List[Dict], generated_df: pl.DataFrame
    ) -> List[Dict]:
        """Build per-trajectory metadata with clear labels"""
        metadata = []

        for idx, (scenario, row) in enumerate(
            zip(scenario_data, generated_df.iter_rows(named=True))
        ):
            metadata.append(
                {
                    "index": idx,
                    ...
                    "scenario_details": {
                        k: v for k, v in scenario.items() if k != "scenario_tags"
                    },
                }
            )
    ...
    def _build_trajectory_scenarios_mapping(self, scenario_data: List[Dict]) -> Dict:
        ...
        return {
            "trajectories": [
                {"index": idx, "scenarios": s["scenario_tags"]}
    ...
    def _compute_individual_scenarios(
        self, scenario_data, generated_df, real_df
    ) -> Dict:
        ...
        for tag in sorted(all_tags):  # Sort for consistent output
            indices = [
                i for i, s in enumerate(scenario_data) if tag in s["scenario_tags"]
            ]

            if len(indices) < min_samples:
                logger.info(
                    f"    Skipping '{tag}' - only {len(indices)} samples (min: {min_samples})"
                )
                continue

            logger.info(
                f"    Computing metrics for '{tag}' ({len(indices)} trajectories)..."
            )
    ...
    def _compute_combinations(self, scenario_data, generated_df, real_df) -> Dict:
        ...
        sorted_combos = sorted(
            combination_groups.items(), key=lambda x: len(x[1]), reverse=True
        )[:20]
        ...
        for combo, indices in sorted_combos:
            ...
            logger.info(
                f"    Computing metrics for '{combo_name}' ({len(indices)} trajectories)..."
            )
    ...
    def _compute_hierarchical(self, scenario_data, generated_df, real_df) -> Dict:
        ...
        for parent_tag, child_tags in hierarchies:
            ...
            parent_indices = [
                ...
                if parent_tag in s["scenario_tags"]
            ]
            ...
            for child_tag in child_tags:
                ...
                if len(subset_indices) >= min_samples:
                    logger.info(
                        f"    Computing hierarchical: {parent_tag} → {child_tag} ({len(subset_indices)} trajectories)..."
                    )

                    metrics = self._compute_subset_metrics(
                        subset_indices, generated_df, real_df
                    )
                    breakdown[child_tag] = {
    ...
    def _compute_subset_metrics(
        self, indices: List[int], generated_df, real_df
    ) -> Dict:
        ...
        try:
            ...
            global_metrics = GlobalMetrics(
                real_subset, generated_subset, self.geo_df_pandas
            ).evaluate()
    ...
    def _parse_generated_trajectory_to_tuples(self, row: Dict) -> List[Tuple]:
        ...
        try:
            ...
            if len(road_ids) == len(timestamps):
                ...
                logger.warning(
                    f"Length mismatch: {len(road_ids)} roads, {len(timestamps)} timestamps"
                )
                return None
        except Exception as e:
    ...
    def _run_statistical_tests(
        self, scenario_data, generated_df, real_df, individual_metrics
    ) -> Dict:
        """Run statistical significance tests"""
        if not self.config.analysis["statistics"]["enabled"]:
            return {}

        results = {"significance_tests": {}, "effect_sizes": {}}
        ...
        for tag1, tag2 in comparisons:
            if tag1 in individual_metrics and tag2 in individual_metrics:
                for metric_key in metric_keys:
                    if (
                        metric_key in individual_metrics[tag1]["metrics"]
                        and metric_key in individual_metrics[tag2]["metrics"]
                    ):
                        ...
                        results["significance_tests"][
                            ...
                            "relative_difference": abs(metric1 - metric2)
                            / ((metric1 + metric2) / 2)
                            * 100,
    ...
    if results["individual_scenarios"]:
        ...
        for scenario in scenarios:
            ...
            for metric in metrics:
                value = results["individual_scenarios"][scenario]["metrics"].get(
                    metric, float("nan")
                )
                row.append(value)
            data.append(row)
...
def run_scenario_analysis(
    ...
    geo_df = pl.read_csv(
        geo_file, infer_schema_length=10000, schema_overrides=schema_overrides
    )
    ...
    logger.info(
        f"💾 Saved trajectory metadata ({len(results['trajectory_metadata'])} trajectories)"
    )
    ...
    logger.info(
        f"💾 Saved scenario combinations ({len(results['scenario_combinations'])} combos)"
    )
    ...
    for tag, count in sorted(
        results["scenario_metrics"]["overview"]["scenario_distribution"].items(),
        key=lambda x: x[1],
        reverse=True,
    )[:10]:
        pct = (
            count / results["scenario_metrics"]["overview"]["total_trajectories"]
        ) * 100
        print(f"  {tag:<20} {count:>5} ({pct:>5.1f}%)")

    if results["scenario_metrics"]["statistics"] and results["scenario_metrics"][
        "statistics"
    ].get("significance_tests"):
        print("\nKey Metric Differences:")
        for test_name, test_result in results["scenario_metrics"]["statistics"][
            "significance_tests"
        ].items():
            if "Distance_JSD" in test_name:  # Show only distance metric for brevity
                print(
                    f"  {test_result['group1']} vs {test_result['group2']}: "
...
def run_cross_model_scenario_analysis(
    eval_dir: Path, config_path: Path = None, od_sources: List[str] = None
):
    """
    ...
    results = {"dataset": None, "models": [], "od_sources": od_sources, "analysis": {}}
    ...
    for od_source in od_sources:
        ...
        for model in models:
            ...
            if metadata_file.exists():
                ...
                logger.info(
                    f"    Loaded {model}: {len(models_metadata[model]['trajectories'])} trajectories"
                )
        ...
        for model, metadata in models_metadata.items():
            ...
            for traj in metadata["trajectories"]:
                ...
                for scenario_tag in traj["scenario_tags"]:
                    ...
                    scenario_od_pairs[scenario_tag][od_pair][model].append(
                        traj["index"]
                    )
        ...
        for scenario, od_pairs in scenario_od_pairs.items():
            ...
            for (origin, dest), model_data in od_pairs.items():
                ...
                od_analysis["scenarios"][scenario]["od_pairs"][od_key] = {
                    ...
                    "trajectory_counts": {
                        m: len(indices) for m, indices in model_data.items()
                    },
                }
            ...
            if len(scenarios) >= 2:
                ...
                for scenario in scenarios:
                    if od_pair in scenario_od_pairs[scenario]:
                        models_with_od.update(
                            scenario_od_pairs[scenario][od_pair].keys()
                        )
        ...
        logger.info(
            f"  Found {multi_scenario_count} OD pairs appearing in multiple scenarios"
        )
    ...
    for od_source, analysis in results["analysis"].items():
        ...
        print(
            f"  Multi-scenario candidates: {len(analysis['multi_scenario_candidates'])}"
        )
...
""",
    ...
    parser.add_argument(
        "--real", type=str, help="Real trajectories CSV (required with --generated)"
    )
    parser.add_argument(
        "--geo", type=str, help="Geo data file (required with --generated)"
    )
    parser.add_argument(
        "--output", type=str, help="Output directory (required with --generated)"
    )
    ...
    parser.add_argument(
        "--config",
        type=str,
        help="Scenario config YAML (required for individual model analysis)",
    )
    parser.add_argument(
        "--models",
        type=str,
        help="Comma-separated list of models to analyze (for --eval-dir)",
    )
    parser.add_argument(
        "--od-sources", type=str, default="train,test", help="OD sources to analyze"
    )
    parser.add_argument(
        "--cross-model",
        action="store_true",
    ...
    if args.cross_model:
        ...
        if not all([args.real, args.geo, args.output]):
            parser.error(
                "--real, --geo, and --output are required when using --generated"
            )
        ...
        # Read with proper schema
        geo_df = pl.read_csv(
            args.geo, infer_schema_length=10000, schema_overrides=schema_overrides
        )
        ...
        print("\nScenario distribution:")
        for tag, count in sorted(
            results["overview"]["scenario_distribution"].items(),
            key=lambda x: x[1],
            reverse=True,
        )[:10]:
