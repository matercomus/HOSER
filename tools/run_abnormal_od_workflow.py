...
class AbnormalODWorkflowRunner:
    ...
    def __init__(
        ...
        Args:
            ...
            real_data_dir: Directory containing train.csv and test.csv
        ...
        self.gene_dir = self.eval_dir / "gene_abnormal" / self.dataset / f"seed{self.seed}"
    ...
    def _validate_inputs(self):
        ...
        assert self.eval_dir.exists(), f"Evaluation directory not found: {self.eval_dir}"
        assert self.real_data_dir.exists(), f"Real data directory not found: {self.real_data_dir}"
        ...
        if not self.skip_detection:
            assert self.detection_config, "detection_config is required when skip_detection=False"
            assert Path(self.detection_config).exists(), f"Detection config not found: {self.detection_config}"
    ...
    def run_phase_0_detection(self) -> bool:
        ...
        if self.skip_detection:
            ...
            train_detection = self.abnormal_dir / "train" / "real_data" / "detection_results.json"
            test_detection = self.abnormal_dir / "test" / "real_data" / "detection_results.json"
            
            assert train_detection.exists() and test_detection.exists(), \
                f"Detection results not found. Set skip_detection=False to run detection."
    ...
    def run_phase_3_extract_od_pairs(self) -> Path:
        ...
        total_pairs = sum(len(pairs) for pairs in od_data["od_pairs_by_category"].values())
    ...
    def run_phase_4_generate_trajectories(self) -> Path:
        ...
        logger.info(f"✅ Phase 4 complete: Generated files for {len(generated_files)} models")
    ...
    def run_phase_5_evaluate(self) -> Path:
        ...
        logger.info(f"✅ Phase 5 complete: Evaluation results saved")
    ...
    def run_analysis_and_visualization(self):
        ...
        analyze_wang_results(
            eval_dirs=[self.eval_dir],
            output_file=wang_results_file
        )
    ...
    def _plot_abnormality_reproduction_rates(self, model_results: Dict[str, Any]):
        ...
        # Add value labels
        for i, (bar, rate, count, total) in enumerate(zip(bars, rates, counts, [model_results[m]["total_trajectories"] for m in models])):
            width = bar.get_width()
            ax.text(
                width + 0.5,
        ...
        plt.tight_layout()
        plt.savefig(self.figures_dir / "abnormality_reproduction_rates.png", dpi=300, bbox_inches="tight")
        plt.savefig(self.figures_dir / "abnormality_reproduction_rates.svg", bbox_inches="tight")
        plt.close()
    ...
    def _plot_similarity_metrics(self, model_results: Dict[str, Any]):
        ...
        bars1 = ax.bar(x - width, edr_scores, width, label='EDR', color='#2ecc71', alpha=0.8)
        bars2 = ax.bar(x, dtw_scores, width, label='DTW', color='#3498db', alpha=0.8)
        bars3 = ax.bar(x + width, hausdorff_scores, width, label='Hausdorff', color='#e74c3c', alpha=0.8)
        ...
        plt.tight_layout()
        plt.savefig(self.figures_dir / "similarity_metrics_comparison.png", dpi=300, bbox_inches="tight")
        plt.savefig(self.figures_dir / "similarity_metrics_comparison.svg", bbox_inches="tight")
        plt.close()
    ...
    def _plot_abnormality_by_category(self, model_results: Dict[str, Any]):
        ...
        for i, cat in enumerate(categories):
            ax.bar(models, category_data[cat], bottom=bottom, label=cat, color=colors[i], alpha=0.8)
        ...
        plt.tight_layout()
        plt.savefig(self.figures_dir / "abnormality_by_category.png", dpi=300, bbox_inches="tight")
        plt.savefig(self.figures_dir / "abnormality_by_category.svg", bbox_inches="tight")
        plt.close()
    ...
    def _plot_metrics_comparison_heatmap(self, model_results: Dict[str, Any]):
        ...
        plt.tight_layout()
        plt.savefig(self.figures_dir / "metrics_heatmap.png", dpi=300, bbox_inches="tight")
    ...
    def _create_summary_report(self):
        ...
        summary = {
            ...
            "files": {
                ...
                "wang_visualizations": str(self.eval_dir / "figures" / "wang_abnormality" / self.dataset),
