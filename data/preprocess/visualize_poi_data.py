...
class POIDataVisualizer:
    ...
    def load_data(self):
        ...
        print(
            f"  - {self.density_matrix.shape[0]} zones × {self.density_matrix.shape[1]} categories"
        )

    def load_beijing_boundary(
        self, shapefile_path: str = "/home/matt/gadm41_CHN_shp/gadm41_CHN_3.shp"
    ):
        ...
        try:
                ...
                if field in gdf.columns:
                    for keyword in beijing_keywords:
                        if beijing_mask is None:
                            beijing_mask = gdf[field].str.contains(
            ...
            if beijing_mask is not None and beijing_mask.any():
                ...
                print(
                    f"✅ Found Beijing boundary with {len(self.beijing_boundary)} features"
                )
            else:
    ...
    def load_poi_points(
        self,
        poi_file: str = "/home/matt/POI/2021_combined_utf8_no_bom.csv",
        ...
        try:
            ...
            poi_df = (
                pl.scan_csv(poi_file, encoding="utf8").head(sample_size * 3).collect()
            )
            ...
            print(
                f"✅ Loaded {len(self.poi_points['lon'])} POI points for visualization"
            )
            print(
                f"   Latitude range: {self.poi_points['lat'].min():.3f} to {self.poi_points['lat'].max():.3f}"
            )
    ...
    def plot_poi_category_distribution(self):
        ...
        bars = ax.bar(
            range(len(sorted_categories)), sorted_totals, color="steelblue", alpha=0.7
        )
        ax.set_xlabel("POI Category", fontsize=12)
        ax.set_ylabel("Total POI Count (number of POIs)", fontsize=12)
        ax.set_title(
            "POI Distribution by Category (Sorted by Count)",
            fontsize=14,
            fontweight="bold",
        )
        ...
        for bar, value in zip(bars, sorted_totals):
            ax.text(
                ...
                bar.get_height() + sorted_totals.max() * 0.01,
                f"{int(value):,}",
                ha="center",
                va="bottom",
                fontsize=9,
        ...
        plt.tight_layout()
        plt.savefig(
            self.data_dir / "poi_category_distribution.pdf",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def plot_zone_poi_density(self):
        ...
        fig.suptitle(
            "POI Density Analysis Across HOSER Zones",
            fontsize=16,
            fontweight="bold",
            y=0.95,
        )
        ...
        ax1.axvline(
            np.median(zone_totals[zone_totals > 0]),
            color="orange",
            linestyle="--",
            linewidth=2,
        ...
        top_10_indices = np.argsort(category_totals)[-10:][
            ::-1
                ...
                filtered_stats.append(
                    {
                        ...
                        "max_filtered": np.max(filtered_data)
        ...
        ax2.set_title(
            "POI Distribution by Category (Top 10 Categories, Outliers Filtered)",
            fontsize=14,
            fontweight="bold",
        )
        ...
        if self.hoser_zones and self.beijing_boundary is not None:
            ...
            zones_gdf = gpd.GeoDataFrame(
                ...
                geometry=[
                    Point(xy)
                    for xy in zip(
                        [z["lon"] for z in zone_data], [z["lat"] for z in zone_data]
                    )
                ],
                crs="EPSG:4326",
            )
            ...
            self.beijing_boundary.boundary.plot(
                ax=ax3, color="black", linewidth=2, alpha=0.8
            )
            ...
            ax3.set_title(
                "Geographic Distribution of POI Density", fontsize=14, fontweight="bold"
            )
        ...
        plt.tight_layout()
        plt.savefig(
            self.data_dir / "zone_poi_density.pdf", dpi=300, bbox_inches="tight"
        )
        print(
            f"✅ Zone POI density plot with geographic context saved to {self.data_dir / 'zone_poi_density.pdf'}"
        )
        plt.close()
    ...
    def plot_poi_density_heatmap(self):
        ...
        fig.suptitle(
            "Geographic POI Density Analysis", fontsize=16, fontweight="bold", y=0.95
        )

        # Plot 1: Geographic POI Density Surface
        self.beijing_boundary.boundary.plot(
            ax=ax1, color="black", linewidth=2, alpha=0.8
        )
        ...
        density_surface = griddata(
            points, values, (lon_mesh, lat_mesh), method="cubic", fill_value=0
        )
        ...
        ax1.set_title(
            "POI Density Surface (Interpolated)", fontsize=14, fontweight="bold"
        )
        ax1.grid(True, alpha=0.3)
        ...
        self.beijing_boundary.boundary.plot(
            ax=ax2, color="black", linewidth=2, alpha=0.8
        )
        ...
        plt.tight_layout()
        plt.savefig(
            self.data_dir / "poi_density_heatmap.pdf", dpi=300, bbox_inches="tight"
        )
        print(
            f"✅ Geographic POI density heatmap saved to {self.data_dir / 'poi_density_heatmap.pdf'}"
        )
        plt.close()
    ...
    def plot_zone_geographic_distribution(self):
        ...
        fig.suptitle(
            "Spatial Distribution Analysis: POI Data and HOSER Zones in Beijing",
            fontsize=16,
            fontweight="bold",
            y=0.95,
        )
        ...
        if self.poi_points is not None and len(self.poi_points["lon"]) > 0:
            ...
            ax2.set_ylabel("Latitude (degrees)", fontsize=12)
            ax2.set_title(
                f"POI Points Distribution (n={len(poi_clipped):,})",
                fontsize=14,
                fontweight="bold",
            )
            ax2.grid(True, alpha=0.3)
        ...
        ax3.set_ylabel("Latitude (degrees)", fontsize=12)
        ax3.set_title(
            f"HOSER Zone Coverage (n={len(zones_gdf)})", fontsize=14, fontweight="bold"
        )
        ax3.grid(True, alpha=0.3)
        ax3.legend()
    ...
    def plot_category_correlation(self):
        ...
        plt.title(
            "POI Category Correlation Matrix (Sorted by Correlation Strength)",
            fontsize=16,
            fontweight="bold",
            pad=20,
        )
        ...
        plt.tight_layout()
        plt.savefig(
            self.data_dir / "poi_category_correlation.pdf", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def generate_summary_report(self):
        ...
        top_categories = sorted(
            zip(self.poi_categories, category_totals), key=lambda x: x[1], reverse=True
        )
