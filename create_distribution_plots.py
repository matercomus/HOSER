...
class DistributionPlotter:
    ...
    def _calculate_haversine(self, lon1: float, lat1: float, 
    ...
    def _load_real_data(self, csv_path: Path, sample_size: int = 50000) -> Dict[str, List[float]]:
        """Load and compute metrics from real data"""
        logger.info(f"📂 Loading real data from {csv_path.name} (sample: {sample_size})...")
    ...
    def plot_distance_distributions(self):
        ...
        for csv_file in sorted(self.gene_dir.glob("*.csv")):
            if 'vanilla_train' in csv_file.name:
                ...
                generated_data['distilled_seed44_train'] = self._load_generated_data(csv_file)
            elif 'distilled_seed44_test' in csv_file.name:
                generated_data['distilled_seed44_test'] = self._load_generated_data(csv_file)
    ...
    def _plot_distance_comparison(self, real_distances: List[float], 
    ...
    def _plot_radius_comparison(self, real_radii: List[float], 
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate distribution comparison plots",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Generate distribution plots from evaluation directory
  uv run python create_distribution_plots.py --eval-dir hoser-distill-optuna-6
  
  # Specify dataset explicitly
  uv run python create_distribution_plots.py --eval-dir eval_xyz --dataset Porto
        """
    )
