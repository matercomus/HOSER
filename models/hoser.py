...
class HOSER(nn.Module):
    def __init__(
        ...
        self.road_network_encoder = RoadNetworkEncoder(
            road_network_encoder_config, road_network_encoder_feature
        )
    ...
    def setup_road_network_features(self):
        self.all_road_embedding_after_gnn = (
            self.road_network_encoder.get_road_embedding()
        )
        self.all_zone_embedding_after_gnn = (
            self.road_network_encoder.get_zone_embedding()
        )
    ...
    def forward(
        ...
        zone_embedding = self.all_zone_embedding_after_gnn[
            self.road2zone[trace_road_id]
        ]
        ...
        destination_zone_embedding = self.all_zone_embedding_after_gnn[
            self.road2zone[destination_road_id]
        ]
        candidate_road_embedding = self.all_road_embedding_after_gnn[candidate_road_id]

        logits, time_pred = self.navigator(
            trajectory_embedding,
            destination_zone_embedding,
            candidate_road_embedding,
            metric_distance,
            metric_angle,
        )
    ...
    def infer(
        ...
        logits, time_pred = self.navigator.infer(
            trajectory_embedding,
            destination_zone_embedding,
            candidate_road_embedding,
            metric_distance,
