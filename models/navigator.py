...
class Navigator(nn.Module):
    ...
    def forward(
        ...
        q = torch.cat(
            [
                trajectory_embedding,
                destination_zone_embedding.unsqueeze(1).expand(
                    -1, trajectory_embedding.size(1), -1
                ),
            ],
            dim=-1,
        ).unsqueeze(2)
        k = torch.cat(
            [
                candidate_road_embedding,
                self.metric_distance_proj(metric_distance.unsqueeze(-1)),
                self.metric_angle_proj(metric_angle.unsqueeze(-1)),
            ],
            dim=-1,
        )

        logits = self.v_proj(torch.tanh(self.q_proj(q) + self.k_proj(k))).squeeze(-1)
        time_pred = self.time_estimator_linear_3(
            F.gelu(
                torch.cat(
                    [
                        self.time_estimator_linear_1(
                            trajectory_embedding.unsqueeze(2).expand(
                                -1, -1, candidate_road_embedding.size(2), -1
                            )
                        ),
                ...
                torch.cat(
                    [
                        self.time_estimator_linear_1(
                            trajectory_embedding[:, -1, :]
                            .unsqueeze(1)
                            .expand(-1, candidate_road_embedding.size(1), -1)
                        ),
                        self.time_estimator_linear_2(candidate_road_embedding),
                    ],
                    dim=-1,
