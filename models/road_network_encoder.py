...
class RoadNetworkEncoder(nn.Module):
    def __init__(self, config, feature):
        ...
        assert (
            config.road_id_emb_dim
            + config.len_emb_dim
            + config.type_emb_dim
            + config.lon_emb_dim
            + config.lat_emb_dim
            == config.intersection_emb_dim
            == config.zone_id_emb_dim
        )
        ...
        self.road_id_emb = nn.Embedding(
            config.road_id_num_embeddings, config.road_id_emb_dim
        )
        ...
        self.zone_id_emb = nn.Embedding(
            config.zone_id_num_embeddings, config.zone_id_emb_dim
        )

        self.road_gat = GAT(
            in_channels=config.road_id_emb_dim
            + config.len_emb_dim
            + config.type_emb_dim
            + config.lon_emb_dim
            + config.lat_emb_dim,
        ...
        self.register_buffer(
            "road_edge_index", torch.from_numpy(feature.road_edge_index)
        )
        self.register_buffer(
            "intersection_attr", torch.from_numpy(feature.intersection_attr)
        )
        ...
        self.register_buffer(
            "zone_edge_index", torch.from_numpy(feature.zone_edge_index)
        )
        self.register_buffer(
            "zone_edge_weight", torch.from_numpy(feature.zone_edge_weight)
        )
    ...
    def get_road_embedding(self):
        ...
        all_road_embedding = torch.cat(
            [all_road_id_embedding, all_road_attr_embedding], dim=1
        )
        all_road_embedding_after_gnn = self.road_gat(
            all_road_embedding, self.road_edge_index, edge_attr=self.intersection_attr
        )
        return all_road_embedding_after_gnn

    def get_zone_embedding(self):
        all_zone_embedding = self.zone_id_emb.weight
        all_zone_embedding_after_gnn = self.zone_gcn(
            all_zone_embedding, self.zone_edge_index, edge_weight=self.zone_edge_weight
        )
