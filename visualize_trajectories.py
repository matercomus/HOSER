...
class VisualizationConfig:
    ...
    # Sampling strategy
    sample_strategy: str = (
        "length_based"  # "random", "length_based", "representative", "scenario"
    )
    ...
    include_real_in_cross_model: bool = (
    ...
    basemap_style: str = (
        "none"  # "osm", "gaode", "cartodb", "none" - deprecated, use road network
    )
...
class RoadNetworkLoader:
    ...
    def load(self) -> Dict[int, List[Tuple[float, float]]]:
        ...
        df = pl.read_csv(
            self.roadmap_path, schema_overrides={"lanes": pl.Utf8, "oneway": pl.Utf8}
        )
...
class TrajectoryLoader:
    ...
    def __init__(
        ...
        road_network: Dict[int, List[Tuple[float, float]]],
    ):
        self.config = config
        self.road_network = road_network

    def load_generated_trajectories(
        self, csv_path: Path, model: str, od_type: str
    ) -> List[Trajectory]:
    ...
    def load_real_trajectories(
        self, csv_path: Path, od_type: str, sample_size: int = 100
    ) -> List[Trajectory]:
        ...
        if sample_size is None:
            ...
            logger.info(
                f"📂 Loading real trajectories from {csv_path.name} (sampling {sample_size})"
            )
            # Read with sampling
            df = pl.read_csv(
                csv_path, n_rows=sample_size * 10
            )  # Read more to account for failures
...
class TrajectorySampler:
    ...
    def sample(
        self, trajectories: List[Trajectory], strategy: str = None
    ) -> Dict[str, Trajectory]:
    ...
    def sample_random(self, trajectories: List[Trajectory]) -> Dict[str, Trajectory]:
        ...
        sampled = random.sample(
            trajectories, min(self.config.samples_per_type, len(trajectories))
        )
    ...
    def sample_by_length(self, trajectories: List[Trajectory]) -> Dict[str, Trajectory]:
        ...
        logger.info(
            f"📊 Sampled trajectories: short={p25:.4f}, medium={p50:.4f}, long={p75:.4f}"
        )
        return result
    ...
    def sample_representative(
        ...
        median_idx = min(
            range(len(lengths)), key=lambda i: abs(lengths[i] - median_length)
        )
    ...
    def sample_by_scenario(
        ...
        if not self.scenario_results or not trajectories:
            logger.warning(
                "No scenario results available, falling back to length-based sampling"
            )
            return self.sample_by_length(trajectories)
        ...
        if not trajectory_mapping or "trajectories" not in trajectory_mapping:
            logger.warning(
                "No trajectory mapping found, falling back to length-based sampling"
            )
            return self.sample_by_length(trajectories)
        ...
        if not scenario_dist:
            logger.warning(
                "No scenario distribution found, falling back to length-based sampling"
            )
            return self.sample_by_length(trajectories)

        # Sort scenarios by count (most common first)
        sorted_scenarios = sorted(
            scenario_dist.items(), key=lambda x: x[1], reverse=True
        )

        # Take top N scenarios
        top_scenarios = [
            s[0] for s in sorted_scenarios[: self.config.max_scenarios_to_plot]
        ]

        logger.info(
            f"Sampling from top {len(top_scenarios)} scenarios: {', '.join(top_scenarios)}"
        )
        ...
        for scenario in top_scenarios:
            ...
            if trajs:
                ...
                logger.info(
                    f"  📍 {scenario}: sampled trajectory with length {trajs[median_idx].length:.4f}"
                )

        if not sampled:
            logger.warning(
                "No trajectories found for top scenarios, falling back to length-based sampling"
            )
            return self.sample_by_length(trajectories)
...
class BasemapManager:
    ...
    def _get_provider(self):
        ...
        if self.config.basemap_style == "cartodb":
            ...
            logger.warning(
                "⚠️  Gaode uses GCJ-02 coordinates, expect ~50-500m offset from WGS84 data"
            )
            try:
                return cx.providers.Gaode.Normal
            except AttributeError:
                logger.warning(
                    "⚠️  Gaode provider not available, falling back to CartoDB"
                )
                return cx.providers.CartoDB.Positron
        else:
            return None
    ...
    def test_connectivity(self) -> bool:
        ...
        try:
            ...
            test_url = (
                provider_url.replace("{z}", "0").replace("{x}", "0").replace("{y}", "0")
            )
    ...
    def add_basemap_safe(self, ax, **kwargs) -> bool:
        ...
        if self.basemap_available is None:
            logger.info(
                f"🌐 Testing {self.config.basemap_style} basemap connectivity..."
            )
        ...
        try:
            ...
            if self.basemap_available is not False:
                logger.info(
                    f"⚠️  Basemap failed ({type(e).__name__}), disabling for remaining plots"
                )
...
class TrajectoryPlotter:
    ...
    def _plot_road_network(
        ...
        if bounds is None:
            ...
            for segment in self.road_network:
                ...
                if (
                    min(seg_lons) <= max_lon + margin
                    and max(seg_lons) >= min_lon - margin
                    and min(seg_lats) <= max_lat + margin
                    and max(seg_lats) >= min_lat - margin
                ):
    ...
    def _convert_coords_for_basemap(
    ...
    def plot_overlaid(
        self, trajectories: Dict[str, Trajectory], output_path: Path, title: str = None
    ):
...
class TrajectoryVisualizer:
    ...
    def _load_scenario_results(self) -> Optional[dict]:
        """Load scenario analysis results from new focused file structure"""
        scenarios_dir = self.config.eval_dir / "scenarios"
        if not scenarios_dir.exists():
            return None

        # Try to load cross-model analysis first (preferred for visualization)
        cross_model_file = scenarios_dir / "cross_model_analysis.json"
        if cross_model_file.exists():
            try:
                with open(cross_model_file) as f:
                    cross_model_data = json.load(f)

                # Load trajectory mapping from first available model for sampling
                for od_dir in scenarios_dir.iterdir():
                    if od_dir.is_dir() and not od_dir.name.startswith("."):
                        for model_dir in od_dir.iterdir():
                            if model_dir.is_dir():
                                mapping_file = model_dir / "trajectory_scenarios.json"
                                if mapping_file.exists():
                                    with open(mapping_file) as f:
                                        cross_model_data["trajectory_mapping"] = (
                                            json.load(f)
                                        )
                                    break
                        if "trajectory_mapping" in cross_model_data:
                            break

                logger.info("✅ Loaded cross-model scenario analysis")
                return cross_model_data
            except Exception as e:
                logger.warning(f"Failed to load cross-model analysis: {e}")

        # Fallback: load from first available model (backward compatibility)
        for od_dir in scenarios_dir.iterdir():
            if od_dir.is_dir() and not od_dir.name.startswith("."):
                for model_dir in od_dir.iterdir():
                    if model_dir.is_dir():
                        ...
                        if metrics_file.exists() and mapping_file.exists():
                            try:
                                ...
                                # Construct compatible structure
                                return {
                                    "overview": metrics.get("overview", {}),
                                    "trajectory_mapping": mapping,
                                }
                            except Exception as e:
                                logger.warning(
                                    f"Failed to load scenario results from {model_dir}: {e}"
                                )
    ...
    def _generate_separate_plots(
        self, sampled: Dict[str, Trajectory], model: str, od_type: str
    ):
        ...
        for od_type in ["train", "test"]:
            ...
            for model_info in gene_files:
                if model_info["od_type"] == od_type:
                    ...
                    trajectories = self.loader.load_generated_trajectories(
                        csv_path, model, od_type
                    )
            ...
            if self.config.include_real_in_cross_model:
                ...
                real_csv = (
                    ...
                    if od_type == "train"
                    else self.config.test_csv_path
                )
                ...
                real_trajectories = self.loader.load_real_trajectories(
                    real_csv, od_type, sample_size=50000
                )
    ...
    def _load_all_models_for_od(
        ...
        if self.config.include_real_in_cross_model:
            ...
            real_trajectories = self.loader.load_real_trajectories(
                real_csv, od_type, sample_size=None
            )
    ...
    def _plot_matching_od_pairs(
        ...
        if not all_od_pairs:
            logger.warning(
                f"⚠️  No common OD pairs found across all models for {od_type} OD"
            )
            return
        ...
        sampled_od_pairs = self._sample_od_pairs_for_comparison(
            list(all_od_pairs), od_indices, models_data
        )
        ...
        if max_plots is not None and len(sampled_od_pairs) > max_plots:
            ...
            logger.info(
                f"📊 Selected {len(sampled_od_pairs)} OD pairs for visualization (limited to max {max_plots})"
            )
        else:
            logger.info(
                f"📊 Selected {len(sampled_od_pairs)} OD pairs for visualization (captures edge cases from analysis)"
            )
        ...
        for i, od_pair in enumerate(sampled_od_pairs):
            ...
            for model_name in sorted(models_data.keys()):
                if (
                    od_pair in od_indices[model_name]
                    and od_indices[model_name][od_pair]
                ):
    ...
    def _track_od_pairs_for_multi_scenario(
        self, models_data: Dict, scenario: str, od_scenario_map: Dict
    ):
    ...
    def _generate_scenario_cross_model_comparisons(self, gene_files: List[Dict]):
        ...
        if (
            not self.scenario_results
            or "trajectory_mapping" not in self.scenario_results
        ):
            logger.warning(
                "No scenario mapping available for scenario cross-model mode"
            )
            return

        logger.info("\n🎯 Generating scenario-based cross-model comparisons...")

        # Note: Statistics are now computed and saved by analyze_scenarios.py
        # This method focuses purely on visualization
        ...
        for od_type in ["train", "test"]:
            ...
            for scenario in scenarios:
                ...
                for model_name, all_trajectories in models_data.items():
                    indices = [
                        t["index"]
                        for t in trajectory_mapping
                        if scenario in t["scenarios"]
                        and t["index"] < len(all_trajectories)
                    ]

                    if indices:
                        scenario_models_data[model_name] = [
                            all_trajectories[i] for i in indices
                        ]
                        logger.info(
                            f"    {model_name}: {len(scenario_models_data[model_name])} trajectories"
                        )

                if len(scenario_models_data) < 2:
                    logger.warning(
                        f"    Skipping {scenario} - not enough models with data"
                    )
                    continue
